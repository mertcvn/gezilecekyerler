class IstanbulNearbyPlaces {
  ...
  async performSearch() {
    ...
    if (!query) {
      this.showToast('Lütfen bir semt veya ilçe adı girin.');
      return;
    }
  ...
  applyFiltersAndRender() {
    ...
    filtered.sort((a, b) => {
      if (this.elements.sortBy.value === 'name')
  ...
  renderList(places) {
    ...
    this.elements.placesList.innerHTML = places
      .map(
        (p, i) => `
  ...
  openDetailModal(place) {
    document.getElementById('modalImage').src =
      place.imageUrl ||
      'https://images.unsplash.com/photo-1524231757912-21f4fe3a7200?w=500&h=300&fit=crop';
  ...
  useGPS() {
    if (!navigator.geolocation) {
      this.showToast('Tarayıcınız GPS desteklemiyor.');
      return;
    }
  ...
  renderHistory() {
    ...
    this.elements.historyTags.innerHTML = history
      .map((h) => `<span class="history-tag" role="button" tabindex="0">${h}</span>`)
      .join('');
